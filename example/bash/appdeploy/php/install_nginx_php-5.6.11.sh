...
if [ ! -f php-5.6.11.tar.gz ];then
  wget http://aliyun-dep.oss-cn-beijing.aliyuncs.com/php/php-5.6.11.tar.gz
fi
...
make install
cd ..
cp ./php-5.6.11/php.ini-production /aliyun/server/php/etc/php.ini
...
#self start
install -v -m755 ./php-5.6.11/sapi/fpm/init.d.php-fpm  /etc/init.d/php-fpm
